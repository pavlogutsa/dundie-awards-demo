...
### 1. Missing Event Field in Activity Creation

**Location:** `src/main/java/com/ninjaone/dundie_awards/service/ActivityService.java`

**Issue:** The `Activity` entity has an `event` field, but `CreateActivityRequest` doesn't include it, and `ActivityService.createActivity()` doesn't set it.
...
public ActivityDto createActivity(CreateActivityRequest req) {
    ...
    // ❌ event field is never set!
    
    return activityMapper.toDto(activityRepository.save(a));
}
```

**Impact:** Activities are created without event information, which may be required for business logic.

**Recommendation:**
- Add `event` field to `CreateActivityRequest` with validation
- Set the event in `ActivityService.createActivity()`
- Or remove the `event` field from `Activity` entity if not needed

### 2. Magic Strings for Activity Events

**Location:** `src/main/java/com/ninjaone/dundie_awards/model/Activity.java`

**Issue:** Activity event type is still a plain string with no validation or type safety.

**Current Code:**
```java
@Column(name = "event")
private String event;  // ❌ Magic string
```

**Impact:** Typos, inconsistent values, no type safety, difficult to refactor.
